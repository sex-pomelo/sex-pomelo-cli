        ...
        try{
          ...
          if( fails.length > 0 ){
            for(let i=0;i<fails.length;i++){
              console.log(`x ${fails[i]} has error!`);
            }
            console.log(`total fails count ${fails.length}! Maybe system time is out of sync`);            
          }
      ...
      (async ()=>{
        try{
          if( Context === 'all' ){
            ...
            let getFrontend = (comd === 'connections')?true:null;
            agent.redis_sendCmdMulti(rl,client, true,getFrontend,comd,cmd,( resData, info)=>{
