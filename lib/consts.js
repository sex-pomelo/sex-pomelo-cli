...
consts.HELP_LOGIN = [
	...
	"pomelo-cli ~/pomelo/pomelo-cli.json",
	"Default type pomelo-cli equals to:",
	"pomelo-cli ${pomelo-cli path}/lib/startup.json\n"
];
...
consts.COMANDS_ALL = [
	...
	["stop", "  stop server. Takes serverId or serverType as argument"],
...
consts.COMANDS_MAP = {
	...
	"show": ["\nshow infos like : servers, connections",
			...
			"servers, status, connections, proxy, handler, components, settings",
			"example: show servers",
			"example: show connections",
			"example: show proxy",
			"example: show handler",
			"example: show components",
	...
	"stop": ["\nstop server. takes serverId as argument.",
			"stop <serverId>|serverType=<serverType>|all",
			"example: stop all",
			"example: stop area-server-1",
			"example: stop serverType=connector\n"
	],
	"kill": ["\nkill all servers.",
			"kill <serverId>|serverType=<serverType>|all",
			"example: kill all",
			"example: kill area-server-1",
			"example: kill serverType=connector\n"
	],
